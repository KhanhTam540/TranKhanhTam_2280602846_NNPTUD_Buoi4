...
// Dữ liệu mẫu đầy đủ từ yêu cầu ban đầu
let data = [
  {
    "id": 1,
  ...
  },
  {
    ...
    "price": 73,
    "description": "Indulge in the essence of summer with this vibrant citrus-scented Expression Eau de Parfum. Encased in a sleek glass bottle with a bold orange cap, this fragrance embodies freshness and elegance. Perfect for daily wear, it's an olfactory delight that leaves a lasting, zesty impression.",
    "category": {
      "id": 5,
      "name": "Miscellaneous",
      "slug": "miscellaneous",
 ...
 * Xử lý query params: title, minPrice, maxPrice, page, limit
 */
router.get('/', function (req, res, next) {
    ...
    if (isNaN(maxPrice)) maxPrice = 1E9;

    if (maxPrice < minPrice) {
        return res.status(400).send({
            "message": "Giá trị maxPrice không được nhỏ hơn minPrice"
        });
    }

    // Kiểm tra tính hợp lệ của page và limit (phải là số nguyên dương, không có dấu chấm thập phân)
    let pageStr = queries.page;
    let limitStr = queries.limit;
    const isPositiveInteger = (val) => val && /^\d+$/.test(val);

    if (pageStr && !isPositiveInteger(pageStr)) {
        return res.status(400).send({ "message": "Tham số page phải là số nguyên dương" });
    }
    if (limitStr && !isPositiveInteger(limitStr)) {
        return res.status(400).send({ "message": "Tham số limit phải là số nguyên dương" });
    }
    ...
    let paginatedResult = result.slice(startIndex, startIndex + limit);

    res.send(paginatedResult);
});

/**
 * 2. TRUY VẤN THEO SLUG (Check equal)
 */
...
/**
 * 4. POST NEW PRODUCT
 * Validate: không để trống, price phải là số, tự động tạo slug chuẩn
 */
router.post('/', function (req, res, next) {
    ...
    if (!title || price === undefined || !description) {
        return res.status(400).send({
            "message": "Các trường title, price, description không được để trống"
        });
    }
    ...
    if (isNaN(numericPrice)) {
        return res.status(400).send({
            "message": "Trường price phải là một số hợp lệ"
        });
    }
...
router.put('/:id', function (req, res, next) {
    ...
    if (result) {
        ...
        if (updateData.title) {
            ...
            result.slug = ConvertTitleToSlug(updateData.title);
        }

        if (updateData.price !== undefined) result.price = parseFloat(updateData.price);
        if (updateData.description) result.description = updateData.description;
...
router.delete('/:id', function (req, res, next) {
    ...
    if (result) {
        ...
        res.send({ "message": "Xóa thành công", "product": result });
    } else {
        res.status(404).send({ "message": "id not found" });
    }
});
